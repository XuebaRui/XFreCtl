...
static void BSP_adc_init(void)
{
  ...
  ADC_InitStruct.ADC_ContinuousConvMode = DISABLE; //连续转换
	...
	ADC_RegularChannelConfig(ADC1, ADC_Channel_1, 1, ADC_SampleTime_55Cycles5); //55.5个采样周期+12.5个转换周期 转换时间为 (55.5+12.5)*(1/9mhz) 7.6us
...
double ADC_GetVal(void)//12位精度电压
{
		double val = 0;
		double adc_val[5];
		int i = 0,j=0;
...
{	
	...
	memset(widget_num,0,sizeof(widget_num));
	
	_tmp = ADC_GetVal(); //将16位分辨率的进行串口滤波
